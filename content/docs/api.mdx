---
title: "Stepper API"
description: "The V1 API keeps the surface small: one root, one ordered list, item primitives, content panels, and previous/next helpers."
eyebrow: "api"
navTitle: "API"
navIcon: "code"
---

import { CodeBlock, InfoGrid, Section } from "@/components/docs-content";
import { PropsTable } from "@/components/props-table";
import { Badge } from "@/components/ui/badge";
import {
  Card,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import {
  apiComponents,
  accessibilityNotes,
  asChildRequirements,
  compositionCode,
  contentProps,
  controlledBehaviorNotes,
  itemProps,
  listProps,
  navigationProps,
  rootProps,
  triggerProps,
  useStepperRows,
  useStepperSnippet,
} from "@/lib/docs";

<Section title="Root props">
  <PropsTable rows={rootProps} />
</Section>

<Section
  title="Controlled fallback"
  description="Controlled values are still normalized to an enabled step so the UI never points at missing or disabled content."
>
  <InfoGrid items={controlledBehaviorNotes} />
</Section>

<Section title="List props">
  <PropsTable rows={listProps} />
</Section>

<Section title="Item props">
  <PropsTable rows={itemProps} />
</Section>

<Section title="Trigger props">
  <PropsTable rows={triggerProps} />
</Section>

<Section
  title="asChild requirements"
  description="StepperTrigger and navigation helpers use Radix Slot composition. The child owns the final element, so it must accept the props Stepper passes."
>
  <InfoGrid items={asChildRequirements} />
</Section>

<Section title="Content props">
  <PropsTable rows={contentProps} />
</Section>

<Section title="Navigation props">
  <PropsTable rows={navigationProps} />
</Section>

<Section
  title="Accessibility model"
  description="Stepper uses progress-step semantics instead of tab semantics, so it can safely compose with forms, route links, and validation gates."
>
  <InfoGrid items={accessibilityNotes} />
</Section>

<Section
  title="useStepper hook"
  description="Use the public hook for custom footers, async validation controls, or buttons outside the visual list."
>
  <div className="flex flex-col gap-4">
    <PropsTable rows={useStepperRows} />
    <CodeBlock code={useStepperSnippet} filename="wizard-footer.tsx" />
  </div>
</Section>

<Section
  title="Primitive parts"
  description="Each part exposes data-slot and data-state attributes for predictable styling."
>
  <div className="grid gap-3 md:grid-cols-2">
    {apiComponents.map((component) => (
      <Card
        key={component.name}
        size="sm"
        className="rounded-none border border-white/10 bg-white/[0.018] shadow-none ring-0"
      >
        <CardHeader>
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col gap-1">
              <CardTitle>{component.name}</CardTitle>
              <CardDescription>{component.description}</CardDescription>
            </div>
            <Badge variant="outline" className="rounded-none border-white/10 bg-[#050505] font-mono text-zinc-500">
              {component.element}
            </Badge>
          </div>
        </CardHeader>
      </Card>
    ))}
  </div>
</Section>

<Section title="Composable trigger">
  <CodeBlock code={compositionCode} filename="composable-trigger.tsx" />
</Section>
